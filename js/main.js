 ...
 * Let’s explore some basic concepts that will help 
 * us create a simple chess AI:
 * 
 * 1. move-generation
 * 2. board evaluation
 * 3. minimax
 ...
 * mostly for teaching purposes. Without tables and its simple interface, 
 * it takes up just 111 lines of code!
 * 
 ...
 *The simplest way to achieve this is to count the relative strength of the pieces on the board 
 * With the evaluation function, we’re able to create an algorithm that 
 * chooses the move that gives the highest evaluation
 */
...
function evaluateBoard (move, prevSum, color) 
{
    ...
    if ('captured' in move)
    {
        // Opponent piece was captured seems decent move of us. Will go for it :)
        if (move.color === color)
        {
            prevSum += (weights[move.captured] + pstOpponent[move.color][move.captured][to[0]][to[1]]);
        }
        // Our piece was captured bad bad bad... :(
        {
    ...
    if (move.flags.includes('p'))
    {
        ...
        // Our piece was promoted ..finally some good news
        if (move.color === color)
        {
            prevSum -= (weights[move.piece] + pstSelf[move.color][move.piece][from[0]][from[1]]);
            prevSum += (weights[move.promotion] + pstSelf[move.color][move.promotion][to[0]][to[1]]);
        }
        // Opponent piece was promoted .. looks bad
        else
        {
            prevSum += (weights[move.piece] + pstSelf[move.color][move.piece][from[0]][from[1]]);
            prevSum -= (weights[move.promotion] + pstSelf[move.color][move.promotion][to[0]][to[1]]);
...
/*
 * minimax algorithm: 
 * https://en.wikipedia.org/wiki/Minimax (pseudocode provided)
 ...
 * 
 * In this algorithm, the recursive tree of all possible moves is explored to a 
 * given depth, and the position is evaluated at the ending “leaves” of the tree.
 * we return either the smallest or the largest value of the child to the parent node, 
 * depending on whether it’s a white or black to move. 
 * (That is, we try to either minimize or maximize the outcome at each level.)
 * maximize the minimum value of the position resulting from the opponent's possible following moves.
 * 
 * Optimization: alpha-beta pruning: 
 * https://en.wikipedia.org/wiki/Alpha%E2%80%93beta_pruning (pseudocode provided)
 * Alpha-beta pruning is an optimization method to the minimax algorithm that allows us 
 * to disregard some branches in the search tree. This helps us evaluate the 
 * minimax search tree much deeper, while using the same resources.
 * The alpha-beta pruning is based on the situation where we can stop evaluating a part of the 
 * search tree if we find a move that leads to a worse situation than a previously discovered move.
...
function minimax(game, depth, alpha, beta, isMaximizingPlayer, sum, color)
{
    ...
    var currMove;
    // Maximum depth exceeded or node is a terminal node -- leaf node
    if (depth === 0 || children.length === 0)
    {
        return [null, sum]
    }

    // Find maximum and minimum from list of the possible moves
 ...
 * This function return the current state of the game. States are:
 * 1. No Check -- Will continue our game play
 * 2. Checkmate -- Oops. someone lost 
 * 3. Match Drawn
...
/*
 *
 * This function returns which player is ahead currently and chances of winning are greater for that player.
 * Well it's a game of chess who knows what's gonna happen next
 * 
*/
...
/*
 * A bit of help for the human we can suggest him/her the winning moves for current postion.
 * Little bit unfair -
*/
